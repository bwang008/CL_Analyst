...
def test_batch_sweep_config_validation():
    ...
    # Seed round-trips through parsing
    assert config.baseline.training_workflow.random_seed == 42


def _valid_manifest_dict():
    """A minimal valid v2 manifest dict for mutation in negative tests."""
    return {
        "_comment": "Test batch sweep",
        "infrastructure": {
            "max_concurrent_vms": 2,
        ...
        "experiments": [
            {"label": "Exp1", "gcs_prefix": "prefix1", "overrides": {}}
        ],
    }


def test_missing_random_seed_is_rejected():
    """USER RULE: a manifest missing random_seed must CRASH, not silently default."""
    manifest = _valid_manifest_dict()
    del manifest["baseline"]["training_workflow"]["random_seed"]
    with pytest.raises(ValidationError):
        BatchSweepConfig.model_validate(manifest)


def test_null_random_seed_is_rejected():
    """An explicit null random_seed must also be rejected (no silent None default)."""
    manifest = _valid_manifest_dict()
    manifest["baseline"]["training_workflow"]["random_seed"] = None
    with pytest.raises(ValidationError):
        BatchSweepConfig.model_validate(manifest)
