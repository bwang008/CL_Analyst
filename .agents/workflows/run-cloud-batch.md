...
Fully-automated GCP Optuna batch: **deploy → monitor → collect → post-optimize → BASELINE artifacts → report**.
...
> **DEFAULT CHAIN CHANGED 2026-07-12 — pass-2 ensemble optimization is OPT-IN.** The
> post-optimizer now runs pass-1 (per-side) → pair selection (Top-4 `top_pairs.json`) →
...
> holdout), promotable configs under `configs/baseline/` and per-side prediction copies
> under `predictions/baseline/` — generated by
> `scripts/generate_baseline_ensemble_artifacts.py` on the optimizer VM (re-runnable
> locally with `--batch-dir`; `--render-optimized` renders the readable companion of the
> optimized summaries for opt-in runs).
...
|------------|--------|-----------|----------|-------|
| **`individual`** (default) | 1 (per-side) + baseline graft | `unified_pair_optimizer.py` → **Top 4** (`top_pairs.json`) | per-side `batch_summary_optimized_sharpe.md` **and** `batch_summary_baseline_sharpe.md` + `sharpe_baseline_backtests.md` + `configs/baseline/` + `predictions/baseline/` | Pass 1 optimizes each side; top individuals are paired; the pairs ship with their **grafted pass-1 params** (baseline). Pass-2 joint re-optimization runs ONLY with `-EnsembleOptimization` (adds the `*_ensembles_*` reports, `sharpe_ensemble_backtests.md`, `configs/optimized/`, `predictions/optimized/`). |
| **`ensemble`** | 1 (brute force) | `select_top_ensembles.py` → Top 8 (`top_8_ensembles.json`) | `batch_ensemble_pre_opt.md` + ensemble reports only | Sweeps all long/short combos; skips per-side optimization. Legacy/divergent; no baseline artifacts. |
...
```
Exit 0 required (zero configs found = FAIL). Configs now live under `configs/baseline/`
(default product) and `configs/optimized/` (opt-in pass-2) — the gate must sweep both
subfolders (recursive). Per config it asserts: resolves via
...
├── batch_summary_optimized_sharpe.md            ← per-side individual optimization (pass-1)
├── optimization_results_sharpe.json
├── top_pairs.json                               ← Top 4 ensemble pairs
├── batch_summary_baseline_sharpe.md             ← BASELINE (pass-1 graft) summary (MAIN)
├── sharpe_baseline_backtests.md                 ← full baseline backtest dumps per ensemble
...
| `gcp/vm_post_optimize.sh` | VM-side post-optimizer (parses manifest; pass-1 → pair selection → baseline artifacts; pass-2 only with `--ensemble-optimization`) |
| `scripts/generate_baseline_ensemble_artifacts.py` | BASELINE (pass-1 graft) artifact generator (default product; also `--render-optimized` readable companion) |
