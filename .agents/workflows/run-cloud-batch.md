...
- `gcp/run_sweep_batch.ps1` — production sweep orchestrator
...
- `gcp/gcp_deploy_optimizer.ps1` — deploys post-optimization VM (called automatically)
...
- ~6 rounds × ~30 min/round = ~3 hours wall time for training
- VM setup overhead: ~3–4 min per experiment (startup script)
- Post-optimization: ~90 min on `n2-standard-32` (auto-deployed)
- Total elapsed: ~4.5–5 hours end-to-end
...
## Step 7 — Post-Optimization (Automatic)

After the batch report is generated, the orchestrator **automatically**:
1. Deploys an `n2-standard-32` VM (128GB RAM) with 24 parallel workers
2. Runs `batch_post_optimizer.py` with 1000 trials per target × metric combination
3. Waits for the optimizer VM to self-terminate (polls every 60s, 3hr timeout)
4. Downloads `batch_summary_optimized.md` and `optimization_results.json` to the local batch directory
5. Deletes the optimizer VM
6. Sends a Telegram notification when results are ready

> [!NOTE]
> Post-optimization sends per-task Telegram messages showing target name, best trial number (for convergence assessment), Sharpe ratio, PnL, and drawdown.

To re-run post-optimization manually, see the `/post-optimize` workflow.

---

## Artifact Layout

Each completed batch produces:
```
reports/
  batch_runs/
    batch_20260424_0954/
      ...
      batch_summary_optimized.md     ← generated by batch_post_optimizer.py (auto)
      optimization_results.json      ← raw optimization results (auto)
      manifest.json                  ← frozen manifest snapshot
