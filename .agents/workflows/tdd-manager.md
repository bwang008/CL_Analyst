...
5. **CRITICAL RED PHASE VALIDATION:** Once the Tester reports completion, you MUST run the test suite BEFORE spawning the Coder. 
    * If tests pass: The Tester wrote a flawed (tautological) test. Send a message to the Tester to fix it. Do not spawn the Coder.
    * If tests fail: Capture the failure traceback. This proves the test is valid and "Red". Proceed to Step 2.
...
Execute the test suite to verify the coder's work. 
* Run the fast tests first to preserve context window size: `conda run -n trader python -m pytest tests/ -v --tb=short -m "not slow"`.
* If tests pass, update the dashboard, summarize the results to the user, and terminate.
