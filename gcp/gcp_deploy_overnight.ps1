...
.DESCRIPTION
    One-command deployment for the overnight alpha experiments:
    - Provisions optuna-runner-overnight (48-core, isolated from canary VMs)
    - Uploads all code files + target generation scripts
    - Downloads set_11 data from GCS to VM
    - Launches vm_overnight_alpha.sh in a detached tmux session
    - Auto-shutdown after completion (unless -NoShutdown)
.EXAMPLE
    .\gcp\gcp_deploy_overnight.ps1
    .\gcp\gcp_deploy_overnight.ps1 -ProvisioningModel STANDARD
    .\gcp\gcp_deploy_overnight.ps1 -NoShutdown
#>

param(
    [string]$VmName = "optuna-runner-overnight",
    [string]$MachineType = "n2-highcpu-48",
    [string]$Zone = "us-central1-a",
    [int]$DiskSizeGB = 50,
    [string]$Project = "cltrainer",
    [string]$ProvisioningModel = "STANDARD",
    [string]$GcsDataPath = "gs://cltrainer-optuna-results/data/cl-5m_bk_set_11.parquet",
...
Write-Host " DEPLOY OVERNIGHT THREE-PRONG ALPHA SEARCH" -ForegroundColor Magenta
Write-Host "=====================================================" -ForegroundColor Magenta
...
if (-not $SkipProvision) {
    Write-Host "`n[1/6] Provisioning overnight VM..."
...
gcloud compute ssh $VmName --zone=$Zone --quiet `
    --command="mkdir -p $RemoteProject/agent $RemoteProject/src/live_execution/strategies $RemoteProject/gcp $RemoteProject/scripts $RemoteProject/configs/strategies $RemoteProject/models/optuna_studies $RemoteProject/reports $RemoteHome/data" 2>$null
...
$codeFiles = @(
    ...
    @{ Local = "gcp\vm_overnight_alpha.sh";      Remote = "gcp/" },
...
# --- [5/6] Launch overnight run ---
Write-Host "`n[5/6] Launching overnight alpha search in tmux..."
...
$fixCmd = "sed -i 's/\r$//' $RemoteProject/gcp/vm_overnight_alpha.sh $RemoteProject/scripts/generate_tbm_target.py $RemoteProject/scripts/generate_vol_target.py"
gcloud compute ssh $VmName --zone=$Zone --command=$fixCmd --quiet 2>$null

$launchCmd = "tmux kill-session -t overnight 2>/dev/null; tmux new-session -d -s overnight 'bash $RemoteProject/gcp/vm_overnight_alpha.sh $shutdownFlag --dataset=$datasetName --n-trials=$NTrials'"
gcloud compute ssh $VmName --zone=$Zone --command=$launchCmd --quiet 2>$null

Write-Host "  Overnight pipeline launched!" -ForegroundColor Green

# --- [6/6] Verify ---
Write-Host "`n[6/6] Verifying tmux session..."
Start-Sleep -Seconds 3
$tmuxCheck = gcloud compute ssh $VmName --zone=$Zone `
    --command="tmux has-session -t overnight 2>/dev/null && echo RUNNING" `
    --quiet 2>$null

if ($tmuxCheck -match "RUNNING") {
    Write-Host "  tmux session 'overnight' is active!" -ForegroundColor Green
} else {
    Write-Host "  WARNING: tmux session may not have started." -ForegroundColor Yellow
    Write-Host "  Debug with: gcloud compute ssh $VmName --command='tmux attach -t overnight'"
}

Write-Host ""
Write-Host "=====================================================" -ForegroundColor Green
Write-Host " OVERNIGHT ALPHA SEARCH LAUNCHED" -ForegroundColor Green
Write-Host "=====================================================" -ForegroundColor Green
Write-Host ""
Write-Host "  VM:           $VmName"
Write-Host "  Expected:     ~2-3 hours (3 experiments × 30 trials)"
$gcsOut = "gs://cltrainer-optuna-results/overnight_alpha/"
Write-Host "  GCS output:   $gcsOut"
Write-Host ""
Write-Host "Useful commands:" -ForegroundColor Cyan
Write-Host "  Check status:     gsutil cat ${gcsOut}STATUS.json"
Write-Host "  View live output: gcloud compute ssh $VmName --zone=$Zone --command='tmux attach -t overnight'"
Write-Host "  Download results: gsutil -m cp -r ${gcsOut} ./overnight_alpha_results/"
Write-Host "  Monitor:          .\gcp\gcp_monitor.ps1 -VmName $VmName -GcsPrefix overnight_alpha -PollIntervalSeconds 120"
Write-Host ""
