...
param(
    [string]$VmName = "optuna-runner-scout",
    [string]$MachineType = "n2-highcpu-48",
    [string]$Zone = "us-central1-a",
    [int]$DiskSizeGB = 50,
    [string]$Project = "cltrainer",
    [string]$ProvisioningModel = "STANDARD",
    [string]$GcsDataPath = "gs://cltrainer-optuna-results/data/cl-5m_bk_set_11c.parquet",
...
if ([string]::IsNullOrWhiteSpace($TargetLong) -or [string]::IsNullOrWhiteSpace($TargetShort)) {
    ...
    Send-TelegramAlert "[FAILED] Deploy Failed: Scout Run`n$errMsg"
    exit 1
}
...
gcloud compute ssh $VmName --zone=$Zone --quiet `
    --command="mkdir -p $RemoteProject/agent $RemoteProject/src/features $RemoteProject/src/live_execution/strategies $RemoteProject/gcp $RemoteProject/configs/strategies $RemoteProject/models/optuna_studies $RemoteProject/reports $RemoteHome/data" 2>$null
...
$codeFiles = @(
    ...
    @{ Local = "gcp\vm_scout_run.sh";           Remote = "gcp/" },
...
$trainCutoffFlag = if ($TrainCutoffDate) { " --train-cutoff-date=$TrainCutoffDate" } else { "" }
$launchCmd = "tmux kill-session -t canary 2>/dev/null; tmux new-session -d -s canary 'bash $RemoteProject/gcp/vm_scout_run.sh $shutdownFlag --dataset=$datasetName --strategy=$StrategyConfig --metrics=$Metrics$targetFlags$bucketFlag$prefixFlag$optTrialsFlag$holdoutCutoffFlag$trainCutoffFlag'"
...
if ($tmuxCheck -match "RUNNING") {
    ...
    Send-TelegramAlert "[STARTING] Deploy Success: Scout Run`nVM: $VmName`nGCS: gs://cltrainer-optuna-results/$GcsPrefix/`nCheck status with: .\gcp\gcp_monitor.ps1 -VmName $VmName -GcsPrefix $GcsPrefix"
    ...
    Send-TelegramAlert "[WARNING] Deploy Warning: Scout Run`nVM: $VmName`ntmux session may not have started. Check logs."
}
